...
void arm_split_rifft_q15(q15_t* pSrc, uint32_t fftLen, const q15_t* pATable, const q15_t* pBTable,
                         q15_t* pDst, uint32_t modifier);
...
void arm_rfft_q15(const arm_rfft_instance_q15* S, q15_t* pSrc, q15_t* pDst)
{
    ...
    if (S->ifftFlagR == 1U) {
        ...
        for (i = 0; i < S->fftLenReal; i++) { pDst[i] = pDst[i] << 1U; }
    }
    else {
                        ...
                        q15_t* pDst, uint32_t modifier)
{
    ...
    q15_t *      pDst1 = &pDst[2],
          *pDst2       = &pDst[(4U * fftLen) - 1U - 14]; /* temp pointers for output buffer */
    q15_t const *pSrc1 = &pSrc[2],
                *pSrc2 = &pSrc[(2U * fftLen) - 1U - 14]; /* temp pointers for input buffer */
