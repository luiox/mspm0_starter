...
static float32x4_t arm_bitonic_sort_4_f32(float32x4_t a, uint8_t dir)
{
    ...
    if (dir == (vgetq_lane_f32(a, 0) > vgetq_lane_f32(a, 1))) { SWAP(a, 0, 1); }
    if (dir == (vgetq_lane_f32(a, 2) > vgetq_lane_f32(a, 3))) { SWAP(a, 2, 3); }

    if (dir == (vgetq_lane_f32(a, 0) > vgetq_lane_f32(a, 3))) { SWAP(a, 0, 3); }
    if (dir == (vgetq_lane_f32(a, 1) > vgetq_lane_f32(a, 2))) { SWAP(a, 1, 2); }
                          ...
                          uint32_t blockSize)
{
    ...
    if ((blockSize & (blockSize - 1)) == 0)   // Powers of 2 only
    {
        ...
        if (blockSize == 1)
            ...
            for (i = 0; i < blockSize; i = i + 16) arm_bitonic_sort_16_f32(pSrc + i, pOut + i, dir);
            ...
            counter = counter >> 1;
            for (i = 0; i < counter; i++) arm_bitonic_merge_64_f32(pOut + 64 * i, dir);

            counter = counter >> 1;
            for (i = 0; i < counter; i++) arm_bitonic_merge_128_f32(pOut + 128 * i, dir);

            counter = counter >> 1;
            for (i = 0; i < counter; i++) arm_bitonic_merge_256_f32(pOut + 256 * i, dir);
    ...
    if ((blockSize & (blockSize - 1)) == 0)   // Powers of 2 only
    {
        for (s = 2; s <= blockSize; s = s * 2) {
            for (i = 0; i < blockSize; i = i + s) arm_bitonic_sort_core_f32(pA + i, s, dir);
        }
    }
#endif
}
