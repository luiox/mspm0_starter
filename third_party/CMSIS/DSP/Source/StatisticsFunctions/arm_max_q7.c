...
static void arm_small_blk_max_q7(const q7_t* pSrc, uint8_t blockSize, q7_t* pResult,
                                 uint32_t* pIndex)
{
...
void arm_max_q7(const q7_t* pSrc, uint32_t blockSize, q7_t* pResult, uint32_t* pIndex)
{
    ...
    if (totalSize <= UINT8_MAX) { arm_small_blk_max_q7(pSrc, blockSize, pResult, pIndex); }
    else {
        uint32_t curIdx     = 0;
