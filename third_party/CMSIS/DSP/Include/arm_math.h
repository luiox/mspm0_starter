...
__STATIC_FORCEINLINE uint32_t arm_recip_q31(q31_t in, q31_t* dst, const q31_t* pRecipTable)
{
    ...
    if (in > 0) { signBits = ((uint32_t)(__CLZ(in) - 1)); }
    else {
...
__STATIC_FORCEINLINE uint32_t arm_recip_q15(q15_t in, q15_t* dst, const q15_t* pRecipTable)
{
    ...
    if (in > 0) { signBits = ((uint32_t)(__CLZ(in) - 17)); }
    else {
...
__STATIC_FORCEINLINE q31_t arm_linear_interp_q31(q31_t* pYData, q31_t x, uint32_t nValues)
{
    ...
    if (index >= (int32_t)(nValues - 1)) { return (pYData[nValues - 1]); }
    else if (index < 0) {
        return (pYData[0]);
    }
    else {
...
__STATIC_FORCEINLINE q7_t arm_linear_interp_q7(q7_t* pYData, q31_t x, uint32_t nValues)
{
    ...
    /* 12 bits for the table index */
    /* Index value calculation */
    if (x < 0) { return (pYData[0]); }
    index = (x >> 20) & 0xfff;

    if (index >= (nValues - 1)) { return (pYData[nValues - 1]); }
    else {
        /* 20 bits for the fractional part */
        /* fract is in 12.20 format */
        fract = (x & 0x000FFFFF);
                                               ...
                                               uint32_t blockSize)
{
    ...
    while (i > 0U) {
        ...
        *dst = circBuffer[rOffset];
        ...
        if (dst == dst_end) { dst = dst_base; }

        /* Circularly update rOffset.  Watch out for positive and negative value  */
        rOffset += bufferInc;

        if (rOffset >= L) { rOffset -= L; }
                                                   ...
                                                   q31_t Y)
{
    ...
    /* Care taken for table outside boundary */
    /* Returns zero output when values are outside table boundary */
    if (rI < 0 || rI > (S->numCols - 2) || cI < 0 || cI > (S->numRows - 2)) { return (0); }
